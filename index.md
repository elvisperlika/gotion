...
- **US-01 — Sign up and sign in**: As a user, I want to register and authenticate with my email address and password so that I can securely access Gotion.
...
- **US-08 — Edit Markdown blocks**: As a user, I want to work with an ordered, nestable list of blocks, where each block is a tree node stored as an independent Markdown string, so that content remains modular and structured.
...
- **US-12 — Collaborate in real time**: As a collaborator, I want to see other connected users' avatars and cursors and co-edit content in real time through CRDT-based synchronization (Yjs) over WebSocket so that we can work together without conflicts.
...
- **US-14 — Search the workspace**: As a user, I want to run indexed full-text searches across workspace titles and content so that I can quickly find information.
